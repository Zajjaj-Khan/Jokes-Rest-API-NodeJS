...
app.patch(('/jokes/:id'),(req,res)=>{
  ...
  const existingJoke = jokes.filter((joke)=>joke.id === id);
  const replaceJoke = {
    id:id,
  ...
  const indexOfJoke = jokes.findIndex((joke)=> joke.id === id);
...
app.delete(("/jokes/:id"),(req,res)=>{
  ...
  if(deleteJoke>-1){
    ...
    res.status(200);
      ...
      console.error("No Joke found with that ID");
      res.status(404)
  }
})
//8. DELETE All jokes
app.delete(("/all"),(req,res)=>{
  ...
  if(userKey === masterKey){
    ...
    res.status(404)
    res.json({error:"You are Unauthorized to do that!"})
  }
